...
namespace minirisk
{
    ...
    struct my_ofstream
    {
        ...
        // open file with the name of the string
        ...
        // m_of is a variable of type ofstream
        std::ofstream m_of;
    };
    ...
    struct my_ifstream
    {
        my_ifstream(const string &fn)
            : m_if(fn) // open file with the name of the string
        {
    ...
    inline my_ofstream &operator<<(my_ofstream &os, const double &v)
    {
        ...
        tmp.d = v;
        // output 16 based hexadecimal number instead of the double
        os.m_of << std::hex << setw(16) << setfill('0') << tmp.u << separator;
        return os;
    }
